...
class DataclassesRenderer:
    ...
    def render(self, parsed_query: ParsedQuery):
        ...
        buffer.write('from typing import Any, Callable, Mapping, List, Dict, Optional')
    ...
    def __render_operation(self, parsed_query: ParsedQuery, buffer: CodeChunk, parsed_op: ParsedOperation):
        ...
        with buffer.write_block(f'class {parsed_op.name}:'):
            ...
            with buffer.write_block(f'def execute(cls, {vars_args} on_before_callback: Callable[[Mapping[str, str], Mapping[str, str]], None] = None), custom_headers: Dict[str, str] = None:'):
                buffer.write(f'client = Client(\'{self.config.endpoint}\', headers=custom_headers)')
            ...
            buffer.write('@classmethod')
            with buffer.write_block(f'async def execute_async(cls, {vars_args} on_before_callback: Callable[[Mapping[str, str], Mapping[str, str]], None] = None, custom_headers: Dict[str, str] = None):'):
                buffer.write(f'client = AsyncIOClient(\'{self.config.endpoint}\', headers=custom_headers)')
                buffer.write(f'variables = {variables_dict}')
